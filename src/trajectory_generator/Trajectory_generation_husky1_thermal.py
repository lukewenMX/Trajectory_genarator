...
class MulTraj_Gene(object):
    ...
    def __init__(self,node_name = "MulTraj_gene"):
        ...
        self.info_sub = message_filters.Subscriber("/rgbdt_fusion/resThermal/full",DetectionFull)
    ...
    def info_odom_callback(self,info,odom):
        ...
        if (info.detections.size > 0):
            ...
            if (len(self.velocity) > 2):
                ...
                rospy.loginfo("The Abnormal Object's label detected in husky1 Thermal is %s",Unique_l)
        else:
            rospy.logdebug("HUSKY1 (Thermal):There is no person detected in the current frame!")
